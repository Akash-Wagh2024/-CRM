...
namespace goGst
{
    public partial class Admin_Lead : System.Web.UI.Page
    {
        ...
        private void BindFeedbackData()
        {
            ...
            {
                SqlCommand cmd = new SqlCommand("SELECT * FROM [lead] Where is_delete=0", conn);
        ...
        protected void GridView1_RowDataBound(object sender, GridViewRowEventArgs e)
        {
            ...
            {
                HiddenField hflead_id = (HiddenField)e.Row.FindControl("hflead_id");
                hflead_id.Value = DataBinder.Eval(e.Row.DataItem, "lead_id").ToString();
            }
        }

        private void AddButtonToNextTable()
        {

            Button btnNextTable = new Button();
            btnNextTable.Text = "Next Table Button";
            btnNextTable.Click += new EventHandler(btnNextTable_Click);
        ...
        protected void GridView1_RowDataBound1(object sender, GridViewRowEventArgs e)
        {
            if (e.Row.RowType == DataControlRowType.DataRow)
            {
                ...
                if (e.Row.Cells.Count > statusColumnIndex && e.Row.Cells[statusColumnIndex].Text == "Convert to Customer")
                {
                    ...
                    link.NavigateUrl = "Addnewclient.aspx?lead_id=" + ((HiddenField)e.Row.FindControl("hflead_id")).Value;
        ...
        private void SearchData(string keyword)
        {
            ...
            {
                con.Open();
                string query = "SELECT  * from lead WHERE (whatsapp_no LIKE @keyword OR company_name LIKE @keyword OR email LIKE @keyword OR product_name LIKE @keyword OR status LIKE @keyword OR full_name LIKE @keyword ) AND is_delete = 0";
        ...
        private void BindData(string selectedValue)
        {
            using (SqlConnection conn = new SqlConnection(ConfigurationManager.ConnectionStrings["crm_con"].ToString()))
            {
                try
                {
                    string query = "SELECT * FROM [lead] WHERE is_delete = 0";
